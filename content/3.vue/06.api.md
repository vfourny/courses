...
Commencez par essayer de récupérer un random user via l'endpoint `GET:` [https://randomuser.me/api/](https://randomuser.me/api/)
...
Actuellement, nous utilisons un bouton afin d'appeler la méthode `fetchData` pour récupérer les données. Cependant, il est recommandé d'utiliser les **hooks de cycle de vie** pour effectuer des actions spécifiques à un moment donné.
...
Ici, nous voudrions désormais commencer avec un tableau de users vide, mais au lancement de l'application, faire un appel API et ajouter un user aléatoire.

```vue
onMounted(async () => { await addUser() });
```
