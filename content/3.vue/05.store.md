...
Prenons l'exemple de notre application actuelle. Je souhaite que lors d'un clic sur un utilisateur de la liste, cela modifie mon compteur pour qu'il soit égal à l'âge de la personne cliquée.

Cela peut être fait via des props et des événements, mais cela devient rapidement ingérable. En effet, il va falloir que les informations remontent du composant `<UserCollection />` au composant `App.vue` pour ensuite être redescendues dans le composant `<CounterComponent />`.
