---
title: Introduction
description: ‘Découvrir les concepts frontend et le framework Vue.js’
icon: mdi:door-open
---

## JavaScript pur vs Vue.js

Avant de plonger dans Vue, posons-nous une question simple : pourquoi ne pas rester sur du JavaScript pur ?

Prenons un exemple concret — afficher une liste de cartes et pouvoir en ajouter une :

#### Avec JavaScript pur

```html
<ul id="card-list"></ul>
<button id="add-btn">Ajouter une carte</button>

<script>
  const cards = [‘Pikachu’, ‘Bulbizarre’, ‘Salamèche’]

  function render() {
    const list = document.getElementById(‘card-list’)
    list.innerHTML = ‘’
    cards.forEach((card) => {
      const li = document.createElement(‘li’)
      li.textContent = card
      list.appendChild(li)
    })
  }

  document.getElementById(‘add-btn’).addEventListener(‘click’, () => {
    cards.push(‘Évoli’)
    render() // Il faut penser à rappeler render() manuellement
  })

  render() // Premier rendu
</script>
```

#### Avec Vue.js

```vue
<template>
  <ul>
    <li v-for="card in cards" :key="card">{{ card }}</li>
  </ul>
  <button @click="cards.push(‘Évoli’)">Ajouter une carte</button>
</template>

<script setup lang="ts">
import { ref } from ‘vue’

const cards = ref([‘Pikachu’, ‘Bulbizarre’, ‘Salamèche’])
</script>
```

La différence est immédiatement visible :

| | JavaScript pur | Vue.js |
| --- | --- | --- |
| **Rendu** | Manuel (`innerHTML`, `appendChild`) | Automatique (réactivité) |
| **Synchronisation** | À gérer manuellement | Automatique |
| **Lisibilité** | Code impératif, difficile à suivre | Code déclaratif, proche du HTML |
| **Scalabilité** | Difficile à maintenir en grandissant | Structuré en composants |

Vue adopte un modèle **déclaratif** : vous décrivez **ce que vous voulez afficher** en fonction des données, et Vue se charge de mettre à jour le DOM. Fini la manipulation manuelle du DOM.

## Pourquoi utiliser un framework front-end ?

Lorsque vous développez une application web, vous pouvez choisir de coder votre interface utilisateur en utilisant des méthodes traditionnelles, comme du JavaScript pur ou jQuery. Cependant, ces méthodes peuvent rapidement devenir complexes et difficiles à maintenir à mesure que votre application grandit. Un framework front-end comme Vue.js vous permet de structurer votre code de manière plus efficace et modulaire.

### Les avantages d’un framework front-end

- **Modularité** : Le code est mieux structuré grâce à des composants réutilisables, facilitant ainsi la gestion du projet.
- **Maintenance simplifiée** : Un framework bien conçu facilite la mise à jour de l’application et la correction des bugs.
- **Gain de productivité** : Avec des outils et des conventions intégrées, vous passez moins de temps à configurer et plus de temps à développer des fonctionnalités.
- **Réactivité** : Un framework comme Vue.js rend l’interface utilisateur réactive, c’est-à-dire que l’UI se met à jour automatiquement en fonction des modifications des données.

### Pourquoi utiliser Vue.js ?

- **Facilité d’apprentissage** : Sa syntaxe est intuitive et la documentation est bien fournie.
- **Composants réutilisables** : Vue permet de structurer l’application en petits morceaux réutilisables, facilitant la gestion du code.
- **Réactivité intégrée** : Vue.js met à jour automatiquement l’interface utilisateur en fonction des données, ce qui rend le développement d’interfaces interactives très simple.
- **Écosystème complet** : Vue offre des outils comme Vue Router pour la navigation, Pinia pour la gestion d’état, et Vite pour une compilation rapide.

### Single Page Application (SPA)

Une Single Page Application (SPA) est une application web qui charge dynamiquement le contenu sans recharger la page entière. Cela signifie que l’interface utilisateur est mise à jour en fonction des actions de l’utilisateur sans avoir à recharger la page. Les frameworks front-end comme Vue.js sont souvent utilisés pour créer des SPAs en raison de leur capacité à gérer efficacement les mises à jour de l’interface utilisateur.

```
┌─────────────────────────────────────────────────────────────┐
│                    NAVIGATEUR (Frontend)                    │
│  ┌────────────────────────────────────────────────────────┐ │
│  │  Vue.js SPA                                            │ │
│  │  - Composants réactifs                                 │ │
│  │  - Navigation sans rechargement (Vue Router)           │ │
│  │  - État global (Pinia)                                 │ │
│  └────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────┘
                            ↕ HTTP / WebSocket
┌─────────────────────────────────────────────────────────────┐
│                   SERVEUR (Backend)                         │
│              API REST + Socket.io                           │
└─────────────────────────────────────────────────────────────┘
```

## Installation et configuration

```sh
npm create vue@latest vuejs-tutorial
cd vuejs-tutorial
npm install
npm run dev
```

### Bundler et Vite

Un outil de bundling prend en entrée plusieurs fichiers source (JavaScript, CSS, images, etc.) et les regroupe en fichiers optimisés pour la production.

Vite se démarque particulièrement pour plusieurs raisons :

| Caractéristique                | Description                                                                                                       |
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------- |
| Démarrage ultra-rapide         | Utilise les modules ECMAScript natifs pour servir les fichiers à la demande, sans build complet au démarrage.     |
| Pré-traitement des dépendances | Recourt à esbuild pour pré-bundler rapidement les dépendances.                                                    |
| Hot Module Replacement (HMR)   | Actualise uniquement les modules modifiés, offrant une expérience de développement fluide.                        |
| Configuration minimale         | Fonctionne avec peu de configuration, intégration simple avec Vue, React, etc.                                    |
| Production optimisée           | S’appuie sur Rollup pour générer des bundles performants en production.                                           |

## Initialisation de Vue avec main.ts et App.vue

### main.ts

Le fichier `main.ts` est le point d’entrée de l’application. C’est ici que l’instance Vue est créée, que les plugins sont enregistrés (Pinia, Vue Router, etc.) et que le composant racine est monté dans le DOM.

```ts
// main.ts
import { createApp } from ‘vue’
import App from ‘./App.vue’

const app = createApp(App)
app.mount(‘#app’)
```

Les points clés :

- `createApp(App)` crée une instance Vue à partir du composant racine.
- `mount(‘#app’)` associe l’application à l’élément HTML dont l’id est `app`.

### App.vue

Le fichier `App.vue` est le composant racine. Un fichier `.vue` suit toujours cette structure :

1. `<template>` — structure HTML du composant.
2. `<script setup lang="ts">` — logique TypeScript du composant.
3. `<style scoped>` — styles CSS isolés à ce composant (optionnel).

```vue
<template>
  <header>
    <img alt="Vue logo" src="./assets/logo.svg" width="125" height="125" />
  </header>
  <main>
    <RouterView />
  </main>
</template>

<script setup lang="ts">
// Imports et logique ici
</script>
```

### Rôle de index.html

Le fichier `index.html` est la page HTML principale. Il contient un `<div id="app">` dans lequel Vue injecte l’application.

```html
<!DOCTYPE html>
<html lang="fr">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Vue App</title>
  </head>
  <body>
    <div id="app"></div>
    <script type="module" src="/src/main.ts"></script>
  </body>
</html>
```

## Ce que vous allez apprendre

Ce cours vous guide à travers les concepts essentiels de Vue 3 pour construire une SPA complète :

| Chapitre | Concept |
| --- | --- |
| Les bases | Réactivité, directives, cycle de vie |
| Composants | Props, événements, slots |
| Composables | Pattern `useX`, fetch natif, localStorage |
| Design System | Naive UI et ses composants |
| Store Pinia | Gestion d’état global |
| Appels API | fetch natif, gestion d’erreurs |
| Routing | Vue Router, navigation guards |
| Socket.io | Temps réel, événements |

> **Projet fil rouge :** À l’issue de ce cours, vous serez capables de réaliser le TP **Pokemon TCG SPA** — une application de jeu de cartes en ligne avec authentification, gestion de decks, matchmaking et jeu en temps réel.
