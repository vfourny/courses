...
Cette commande va générer un fichier `index.js` dans le même dossier que le fichier `index.ts`. Vous pouvez ensuite
exécuter ce fichier avec Node.js comme d'habitude. Si vous souhaitez générer les fichiers compilés dans un dossier
spécifique, vous pouvez modifier le fichier `tsconfig.json` en ajoutant la clé `outDir`.
